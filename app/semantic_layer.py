...
TABLES = dedent(f"""\
    ...
    {FQN}.regions     (12 territory rows, sharing 4 region_name values)
      region_id    — PK; each region_name corresponds to ~3 territory rows
      region_name  — one of: 'Northeast', 'Southeast', 'Midwest', 'West'

    {FQN}.sales_reps  (20 rows)
      rep_id     — PK
      rep_name
      region_id  — FK -> regions.region_id (a territory, not a region_name)
      hire_date  DATE
...
METRICS = dedent("""\
    ...
    - target_attainment_revenue:
        SUM(sales.revenue) / NULLIF(SUM(targets.target_revenue), 0)
        targets stores the period as integer columns (quarter 1–4, year), so join
        sales and targets on rep_id AND product_id AND
        targets.year = YEAR(sales.sale_date) AND
        targets.quarter = QUARTER(sales.sale_date).
